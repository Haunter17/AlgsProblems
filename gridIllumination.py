...
def checkIllumination(N, lamps, queries):
    ...
    # print sorted(lightSiteTable.items())
    # print sorted(lightTable.items())
...
def processLightSite(N, lightSite, lightTable, turnOn):
    ...
    while k + lightSiteCol <= N:
        if turnOn:
            lightTable[(lightSiteRow, k + lightSiteCol)] += 1
        else:
            lightTable[(lightSiteRow, k + lightSiteCol)] -= 1
        k += 1
    k = 1
    while lightSiteCol - k > 0:
        if turnOn:
            lightTable[(lightSiteRow, lightSiteCol - k)] += 1
        else:
            lightTable[(lightSiteRow, lightSiteCol - k)] -= 1
        k += 1
    ...
    while lightSiteRow - k > 0 and lightSiteCol + k <= N:
        if turnOn:
            lightTable[(lightSiteRow - k, lightSiteCol + k)] += 1
        else:
            lightTable[(lightSiteRow - k, lightSiteCol + k)] -= 1
        k += 1
    k = 1
    ...
    if turnOn:
        lightTable[(lightSiteRow, lightSiteCol)] += 1
    else:
        lightTable[(lightSiteRow, lightSiteCol)] -= 1
